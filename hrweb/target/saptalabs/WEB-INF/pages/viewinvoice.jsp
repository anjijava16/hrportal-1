				...
				<div id = "optiondiv" style="width: 100%;">
					<table width = "443px" align ="center" class="optionTable">
						...
						<tr>
							...
							<td align="left"  class="fyBillsPaidTd PADDLFT1PX BLD" ><span style="margin-left: 10px;font-weight:bold;">&#x20B9;</span><span id="currenttotalamttd"  style="width: 100px;font-weight:bold;">${totalamount}</span></td>
						</tr>
					</table>
						...
						<tr class="">
							...
							<td class="value">
								<select id="typeofinvoice" class="leftElement selectBox hidden" tabindex="2" style="margin:0; text-align: center;">
									<option value="h" style="text-align: left;">Hour</option>
									<option value="d" style="text-align: left;">Day</option>
									<option value="w" style="text-align: left;">Week</option>
									<option value="m" style="text-align: left;">Month</option>
					...
					<div class="responsive">
					   <table id="invoicetablecontent" class="data">
							   <tr>
								   ...
								   <th class=""><span id="fixedtypeofperiodrate"></span>Rate<span id="slashspan">/</span><span id="typeofperiodrate"></span></th>
		...
		$(document).ready(function(){
		        ...
		        success: function(data) {
					...
					if(successflag == "true"){
						$.each(results, function (i, result) {
							amount = parseInt(amount) + parseInt(result.totalamount);
			...
			var table = $("#invoiceDataTable").dataTable({
				...
				"footerCallback": function ( row, data, start, end, display ) {
		            ...
		            $("#currenttotalamttd").html(pageTotal); 
		        }, 
		        "aoColumns": [ 
                   {sClass: "alignleft"}, 
                   {sClass: "alignleft"},
				...
				if($("#invoicemonth").datepicker().val() == ""){
					...
					if(url != null && url != ""){
				        ...
				        success: function(data) {
							...
							if(successflag == "true"){
								$.each(results, function (i, result) {
									totalAmount = parseInt(totalAmount) + parseInt(result.totalamount);
			...
			$('#typeofinvoice').on('change', function (e){
			 ...
			 $('#amounttype').on('change', function (e){
				 	if($("#amounttype").val() == "i"){
				 		...
				 		var servicetax = parseFloat(totalamount*14)/100;
						...
						if(successflag == "true"){
							$.each(results, function (i, result) {
								...
								if(result.amounttype == "i"){
									...
									$("#totalamount").removeClass("hidden");
									var subtotalamount = result.totalamount - result.servicetax;
									$("#subtotalamount").html(subtotalamount.toFixed(2));
									$("#sub_total").removeClass("hidden");
									$("#servicetax").html((result.servicetax == undefined || result.servicetax == null || result.servicetax.length <= 0) ? "-" : result.servicetax);
									$("#service_tax").removeClass("hidden");
									$("#totalamountunderline").removeClass("hidden");
								...
								$("#totalamount").html((result.totalamount == undefined || result.totalamount == null || result.totalamount.length <= 0) ? "-" : result.totalamount);
			...
			$("#update").click(function(){
				...
				if(validation == false){
					...
					var resourceURL = $("#contextpath").val()+"/invoice/update/"+id+"/"+invoiceno+"/"+invoicedate+"/"+duedate+"/"+totalamount+"/"+servicetax+"/"+projectid+"/"+customerid+"/"+amounttype+"/"+invoicetype+"/"+status+"/"+received_date+"/"+received_amount+"/"+received_amounttype+"/"+received_comments;
					$.ajax({
						url : resourceURL,
						type : 'GET',
						async: false,
			...
			$("#show_showinvoiceList").click(function(){
				...
				if(url != null && url != ""){
			        ...
			        success: function(data) {
						...
						if(successflag == "true"){
							$.each(results, function (i, result) {
								if(result.servicetax > 0 || result.servicetax != "" ){
									amount = parseFloat(amount) + parseFloat(result.totalamount) + parseFloat(result.servicetax);
								}else{
									amount = parseFloat(amount) + parseFloat(result.totalamount)
								}
									
							});
								$("#currenttotalamttd").html(amount.toFixed(2));
								$("#totaldiv").removeClass("hidden");
							}
				        },
				        error: function (xhr, ajaxOptions, thrownError) {
				...
				if(totalperiod != "" && periodperrate != ""){
					...
					$('[id^="netamt_"]').each(function(i, item) {
						...
						if(netamount != "" && netamount != null && netamount != 0){
							...
							var completeLastTotal = lastTotal;
							if($("#amounttype").val() == "i"){
								var servicetax = (parseFloat(completeLastTotal)*14)/100;
								$("#servicetax").html(servicetax.toFixed(2));
								$("#subtotalamount").html(parseFloat(completeLastTotal).toFixed(2));
