...
<%@ include file="head.jsp"%>
	<head>
		<style>
			...
			#invoice_data_td .ui-datepicker-trigger,#invoice_duedata_td .ui-datepicker-trigger{
				margin-top:3px;
			}
...
<body>
		...
		<div id="innerContainer">
				...
				<div id="invoicedetails" class="hidden MRGT10PX">
					...
					<table class="tableContent">
						...
						<tr class="">
							...
							<td class="value hidden" id="invoice_data_td">
								<b style="float:left; height: 20px;"><input id="invoice_data" readonly="readonly" disabled="disabled" type="text" class="inputdisable datePcKview" style="margin-top: 0px;"></b>
							</td>
							<td class="value" id="invoice_datadiv_td">
								<div id="optional_invoice_data" class="value"></div>
							</td>
							<!-- <td class="value">
						...
						<tr class="">
							...
							<td class="value hidden" id="invoice_duedata_td">
								<b style="float:left; height: 20px;"><input id="invoice_duedata" readonly="readonly" disabled="disabled" type="text" class="inputdisable datePcKview" style="margin-top: 0px;"></b>
							</td>
							<td class="value" id="invoice_duedatadiv_td">
								<div id="optional_invoice_duedata" class="value"></div>
							</td>
							<!-- <td class="value">
								<select id="typeofinvoice" class="leftElement selectBox hidden" tabindex="2" style="margin:0; text-align: center;">
									<option value="h" style="text-align: left;">Hourly</option>
						...
						<tr class="">
							<td class="lableContent">Invoice Amount</td>
							<td class="value"><div id="invoice_amount" style="height: 25px; margin:0; text-align: left; font-weight: bold;"></div></td>
						</tr>
			...
			<div id="assetMsg" class="hidden">
				...
				<div class="clearWidth MRGT10PX"><a href="<%=request.getContextPath() %>/externalinvoice/viewexternalinvoice" id="menu_miscellaneous" ><input type="button" value="Done" id="asset" /></a></div>
			</div>	
			...
			<div id="userMsg" class="hidden">
				...
				<div class="clearWidth MRGT10PX">
					<a href="<%=request.getContextPath() %>/externalinvoice/viewinvoice" ><input type="button" value="Done" id="user" /></a>
				</div>
			</div> 
			<div id ="deletedsuccessful_fancybox" class="hidden">
				<h1><b class="saptaColor">Notification</b></h1>
				<div id = "successful_msg_content"></div>
				<a href="<%=request.getContextPath() %>/externalinvoice/viewinvoice" style="text-decoration: none">
	  ...
	  <script>
		$(document).ready(function(){
			...
			$(function() {
			 	$( "#invoice_data,#invoice_duedata,#invoicedate, #duedate, #received_date, #receiveddate" ).datepicker({
					 yearRange: '1950:2100',
			...
			if(fymonthYear != "" && fymonthYear != null){
				...
				url = $("#contextpath").val()+"/externalinvoice/getbymonth/"+fymonthYear;
				var resourceurl = $("#contextpath").val()+"/externalinvoice/getbydateajax/"+fymonthYear;
				$.ajax({
			        url : resourceurl,
			        type : 'GET',
			        dataType : 'json',
				...
				url = $("#contextpath").val()+"/externalinvoice/getlist";
				$(function() {     
		            $( "#invoicemonth" ).datepicker({
						yearRange: '1950:2100',
						defaultDate: new Date(),
			...
			/* 	var resourceURL = $("#contextpath").val()+"/externalinvoice/getinvoicedetails/"+id;
				$.ajax({
					url : resourceURL,
					type : 'GET',
					dataType : 'json',
				...
				var resourceURL = $("#contextpath").val()+"/externalinvoice/retrivebyid/"+id;
				$.ajax({
					url : resourceURL,
					type : 'GET',
					dataType : 'json',
					...
					success: function(data) {
						...
						if(successflag == "true"){
							...
							//	$("#dueamount").val((result.dueamount == undefined || result.dueamount == null || result.dueamount.length <= 0) ? "-" : result.dueamount);
								...
								$("#invoice_amount").html((result.invoiceamount == undefined || result.invoiceamount == null || result.invoiceamount.length <= 0) ? "-" : result.invoiceamount);
								$("#optional_invoice_data").html((result.invoicedate == undefined || result.invoicedate == null || result.invoicedate.length <= 0) ? "-" : result.invoicedate);
								$("#invoice_data").val((result.invoicedate == undefined || result.invoicedate == null || result.invoicedate.length <= 0) ? "-" : result.invoicedate);
								$("#optional_invoice_duedata").html((result.duedate == undefined || result.duedate == null || result.duedate.length <= 0) ? "-" : result.duedate);
								$("#invoice_duedata").val((result.duedate == undefined || result.duedate == null || result.duedate.length <= 0) ? "-" : result.duedate);
			...
			var resourceURL = $("#contextpath").val()+"/externalinvoice/retrivebyprojectid/"+projectid;
				$.ajax({
					url : resourceURL,
					type : 'GET',
					dataType : 'json',
			...
			$("#findByMonth").click(function() {
				if($("#invoicemonth").datepicker().val() == ""){
					...
					url = $("#contextpath").val()+"/externalinvoice/getbydateajax/"+month;
					if(url != null && url != ""){
						$.ajax({
				        url : url,
				        type : 'GET',
					...
					var table = $('#invoiceDataTable').DataTable();
					table.destroy();
					var resourceURL = $("#contextpath").val()+"/externalinvoice/getbymonth/"+month;
			...
			$("#edit").click(function() {
				...
				$("#optional_invoice_data").addClass("hidden");
				$("#optional_invoice_duedata").addClass("hidden");
				
				$("#invoice_duedata_td").removeClass("hidden");
				$("#invoice_data_td").removeClass("hidden");
			...
			$("#update").click(function(){
				...
				var invoicedate = $("#invoice_data").datepicker().val();
				var duedate = $("#invoice_duedata").datepicker().val();
				var amounttype = $("#amounttype").val();
				var status = $("#status").val();
				var invoiceamount=$("#invoice_amount").html();
				...
				if ((invoicedate) > (duedate))  validation = false; 
				...
				if(validation == false){
					...
					var resourceURL = $("#contextpath").val()+"/externalinvoice/update/"+id+"/"+invoiceno+"/"+invoicedate+"/"+duedate+"/"+projectid+"/"+customerid+"/"+amounttype+"/"+invoiceamount+"/"+status+"/"+received_date+"/"+received_amount+"/"+received_amounttype+"/"+received_comments;
					$.ajax({
						url : resourceURL,
						type : 'GET',
						async: false,
						...
						success: function(data){
							...
							if(successflag == "true"){
								...
								var totalamount = $("#receivedamount").val();
								$("#receiveddate").val("");
								$("#receivedamount").val("");
			...
			$("#invocno").click(function() {
				$("body").css("cursor", "progress");
				var resourceURL = $("#contextpath").val()+"/externalinvoice/viewinvoice/"+$("#invocno").val();
				$(this).attr("href", resourceURL);
				$("body").css("cursor", "default");
			});
				
			function invoiceDeleteReloader(){
				var resourceURL = $("#contextpath").val()+"/externalinvoice/getlist";
			...
			$("#show_showinvoiceList").click(function(){
				...
				if($("#invoicemonth").datepicker().val() != ""){
					...
					month =  monthConversion(month)
					month = month.split('/').join('-');
					resourceURL = $("#contextpath").val()+"/externalinvoice/getbymonth/"+month;
					url = $("#contextpath").val()+"/externalinvoice/getbydateajax/"+month;
				}else{
					resourceURL = $("#contextpath").val()+"/externalinvoice/getlist";
					url = $("#contextpath").val()+"/externalinvoice/gettotalamount";
				}
