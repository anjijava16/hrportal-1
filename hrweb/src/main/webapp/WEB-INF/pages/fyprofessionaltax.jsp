...
<html>
	...
	<body>
		...
		<div id="innerContainer">
			<img id="ptpdfreport" class="MRGL10PX" src="<%=request.getContextPath() %>/resources/icons/pdf.png" style="margin: 4px 20px 0 0; float: right; cursor: pointer;"/>
			<!-- <input type="button" value="Report" class="leftElement MRGL10PX" id="ptpdfreport" style="margin: 4px 20px 0px 0px;"/> -->
			...
			<div id="totaldiv1" align="center" style="width:100%">
					<table  class="optionTable" >
						<tr>
							<td class="currenttotalamtrupyatd1 " align="right" style="margin-left:auto;  margin-top:0;" >PT&nbsp;Amount&nbsp;Paid&nbsp;:</td>
	...
	<script>
		$(document).ready(function(){
			$("#pageTitle").html("PT <b class='saptaColor'>Report</b>");
			...
			$(document).on('click', '.fymonthlist' ,function(){
				...
				var resourceURL = $("#contextpath").val()+"/professionaltax/viewfyprofessionaltax/"+monthYearVal;
				$(this).attr("href", resourceURL);
			});
			
			$("#ptpdfreport").click(function() {
				var startDate = $("#startyear").val();
				var endDate = $("#endyear").val();
				document.location.href =  $("#contextpath").val()+"/ptpdfreport?startDate="+startDate+"&endDate="+endDate;
			});
					...
					success: function(data) {
						...
						if(successflag == "true"){
							...
							$.each(results, function (i, result){
								dueDate.push(result.ptcompletemonth);
							});
							var dueAmount = 0;
							var count = 1;
							var flag = 1;
							...
							for(var i=0; i<newDueDate.length; i++){
								...
								var resultCount = 0;
								$.each(results, function (i, result){
										var fullDueDateFormated = stringToDateConversion(result.ptcompletemonth);
							...
							$("#ptpdfreport").removeClass("hidden");
						}else{
							$("#noListAvailable").html(errors);
							
							$("#ptpdfreport").addClass("hidden");
