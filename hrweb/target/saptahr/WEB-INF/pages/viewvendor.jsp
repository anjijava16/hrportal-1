...
<html>
	...
	<body>
		...
		<!--Header End-->
			<div id="contentArea">
				<div align = "left" id="headlist"><h1>Vendor<b class="saptaColor"> List</b></h1></div>
				<div align = "left" id="headdetail" class="hidden MRGT10PX"> <h1>Vendor<b class="saptaColor"> Detail</b></h1></div>
						...
						<tr>
							...
							<td align="left">						
								<select id="findoption" class="leftElement selectBox" tabindex="2" style="width: 200px; margin:0; text-align: center">
									...
									<option value="findbyid" style="text-align: left;">Find By Vendor Id</option>
								</select>
							</td>
						</tr>
						<tr id="vendorsearch" class = "hidden">
							<td align="right" width = "150px"> <sup class="saptaColor">*</sup>Enter Vendor Id</td>
							<td>:</td>
							<td align="left" colspan = "2" style=" padding-left: 0px;" >
								<input name="vendorid" id="vendorid" type="text" style="width: 195px; padding: 0px; margin-left: 7px;"/>
				...
				<div id="vendorList" class="MRGT10PX">
					<table id="vendorgridtable"></table>
					<div id="vendorgridpager"></div>
				</div>
				...
				<div id="vendorDetail" class="hidden MRGT10PX">
					...
					<table class="tableContent">
						...
						<tr class="">
								...
								<td class="value"><input id="vendorcode" disabled="disabled" class="inputdisable" onkeypress="return ValidateAlpha(event)"/></td>
							</tr>
						<tr>
						...
						<tr>
							...
							<td class="value"><input id="pincode" disabled="disabled" maxlength="6" class="inputdisable" onkeypress="return validateNumeric(event)"></td>
						</tr>
						<tr>
							<td class="lableContent">PhoneNo</td>
						...
						<tr>
							<td class="lableContent">MobileNo</td>
	...
	<script>
		$(document).ready(function(){
			...
			jQuery("#vendorgridtable").jqGrid({
				url: url,
				mtype: 'GET',
				ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },                      
				datatype: "json", 
				colNames:['Vendor Code','Vendor Name', 'Address', 'Email'], 
				colModel:[{name:'vendorcode',index:'vendorcode', width:145, resizable :false},
						  {name:'name',index:'name', width:220, resizable :true},
						  {name:'address',index:'address', width:265, resizable :true},
						  {name:'email',index:'email', width:288, resizable :true},],
				...
				rowNum:10, 
				rowList:[10,50,100],
				pager: '#vendorgridpager', 
				viewrecords: true, 
				hidegrid: false,
				width: null,
				shrinkToFit:false, 
				onSelectRow: function(custid){
					vendorDetails(custid);
				}
			});
			jQuery("#vendorgridtable").jqGrid('navGrid','#vendorgridpager',{edit:false, add:false, del:false, search:false});
...
// 							$("#vendorgridtable").trigger("reloadGrid");
			...
			function findbyvendorid(){
				...
				if(count == 0){
					...
					$.fancybox.open('#alertMsg');
					var resourceURL = $("#contextpath").val()+"/vendordetails/get/"+vendorid;
					$("#vendorgridtable").setGridParam({
						url:resourceURL,
						mtype: 'GET',
						ajaxGridOptions: { contentType: 'application/json; charset=utf-8' },                      
			...
			$("#show_vendor").click(function(){
				...
				if($("#findoption").val() != "all"){
					var id = $("#vendorid").val();
					resourceURL = $("#contextpath").val()+"/vendordetails/get/"+id;
				}
					$("#vendorgridtable").setGridParam({
						url:resourceURL,
						mtype: 'GET',
