...
@WebServlet("/tdspdfreport")
public class TDSReportServlet extends BaseServlet {
	...
	try {
			...
			resp.setHeader("Content-Disposition","attachment; filename=\""+startDate+"_"+endDate+"_"+"TDS_Report.pdf\"");
			...
			PdfPCell contenerowcell = new PdfPCell(new Paragraph("TDS report between"+" "+startDate+" and "+endDate, fontitalic10));
			...
			tdslist = new TDSService().getFinancialYearTDSReport(CommonUtil.convertStringToDate(startDate), CommonUtil.convertStringToDate(endDate));
			for (TDSDO TDSDO : tdslist) {
				totalpay = totalpay + (TDSDO.getAmount());
			}
			...
			for (TDSDO TDSDO : tdslist) {
				...
				PdfPCell tds_amount = new PdfPCell(new Paragraph(decimalformat.format(TDSDO.getAmount()), font8));
