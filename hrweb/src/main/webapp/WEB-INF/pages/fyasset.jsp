...
<html>
	...
	<!--Header Start-->
		...
		<img id="assetpdfreport" class="MRGL10PX" src="<%=request.getContextPath() %>/resources/icons/pdf.png" style="margin: 4px 20px 0 0; float: right; cursor: pointer;"/>
		<!-- <input type="button" value="Report" class="leftElement MRGL10PX" id="assetpdfreport" style="margin: 4px 20px 0px 0px;"/> -->
			...
			<div id="totaldiv1" align="center" style="width:100%">
					<table  class="optionTable">
						<tr>
							<td class="currenttotalamtrupyatd1"  align="right" style="margin-left:auto;margin-top:0;">Asset&nbsp;Amount&nbsp;Paid&nbsp;:</td>
						<!-- 	<td class="currenttotalamtrupyatd MGNLFTAUTO" style="margin-left:auto;" class="rupyaINR WebRupee">&#x20B9;</td>
							<td id="currenttotalamttd" class="fyBillsPaidTd PADDLFT1PX BLD MGNLFTAUTO" style="margin-left:auto;"></td> -->
	...
	<script>
		$(document).ready(function(){
			$("#pageTitle").html("Asset <b class='saptaColor'>Report");
			...
			$(document).on('click', '.fymonthlist' ,function(){
				...
				var resourceURL = $("#contextpath").val()+"/asset/viewfyasset/"+monthYearVal;
					...
					success: function(data) {
						...
						if(successflag == "true"){
							...
							$.each(results, function (i, result){
								dueDate.push(result.dateofpurchased);
							});
							var dueAmount = 0;
							var count = 1;
							var flag = 1;
							...
							for(var i=0; i<newDueDate.length; i++){
								...
								var resultCount = 0;
								$.each(results, function (i, result){
										var fullDueDateFormated = stringToDateConversion(result.dateofpurchased);
										if(firstDay <= fullDueDateFormated && fullDueDateFormated <= lastDay){
												dueAmount = parseFloat(dueAmount)+parseFloat(result.price);
												completeTotalAmount = parseFloat(completeTotalAmount)+parseFloat(result.price);
												resultCount++;
										}
								});
								if(resultCount != 0){
							...
							$("#assetpdfreport").removeClass("hidden")
						}else{
							$("#noListAvailable").html(errors);
							
							$("#assetpdfreport").addClass("hidden")
			...
			$("#assetpdfreport").click(function() {
				...
				document.location.href =  $("#contextpath").val()+"/assetpdfreport?startDate="+startdate+"&endDate="+enddate;
