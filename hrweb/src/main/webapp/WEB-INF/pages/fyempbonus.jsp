...
<html>
	...
	<body>
		...
		<div id="innerContainer">
			<img id="tdspdfreport" class="MRGL10PX" src="<%=request.getContextPath() %>/resources/icons/pdf.png" style="margin: 4px 20px 0 0; float: right; cursor: pointer;"/>
			<!-- <input type="button" value="Report" class="leftElement MRGL10PX" id="tdspdfreport" style="margin: 4px 20px 0px 0px;"/> -->
			...
			<div id="totaldiv1" align="center" style="width:100%">
				<table  class="optionTable" >
					<tr>
							<td class="currenttotalamtrupyatd1" align="right" style="margin-left:auto; margin-top:0">TDS&nbsp;Amount&nbsp;Paid&nbsp;:</td>
	...
	<script>
		$(document).ready(function(){
			$("#pageTitle").html("Bouns <b class='saptaColor'>Report</b>");
			...
			$("#findByFinancialYear").click(function() {
			 ...
			 $(document).on('click', '.fymonthlist' ,function(){
				...
				var resourceURL = $("#contextpath").val()+"/empbonus/viewfybonus/"+monthYearVal;
				$(this).attr("href", resourceURL);
			});
			
			$("#tdspdfreport").click(function() {
				var startDate = $("#startyear").val();
				var endDate = $("#endyear").val();
				document.location.href =  $("#contextpath").val()+"/empbonuspdfreport?startDate="+startDate+"&endDate="+endDate;
			});
			 
			function finalYearReportProcess(currentdate, nextYear){
			/* 	$("#block_overlay").removeClass("hidden"); */
				var completeTotalAmount = 0;
				var resourceURL = $("#contextpath").val()+"/fyreports/getempbonusreport/"+currentdate+"/"+nextYear;
				$.ajax({
					url : resourceURL,
					type : 'GET',
					dataType : 'json',
					success: function(data) {
						...
						if(successflag == "true"){
							...
							//$("#fiancialdashboard").html("");
							//dueDate Array
							var dueDate = new Array();
							$.each(results, function (i, result){
								dueDate.push(result.bonusdatemonth);
							});
							var dueAmount = 0;
							var count = 1;
							var flag = 1;
							...
							for(var i=0; i<newDueDate.length; i++){
								...
								var resultCount = 0;
								$.each(results, function (i, result){
										var fullDueDateFormated = stringToDateConversion(result.bonusdatemonth);
								...
								if(resultCount != 0 && completeTotalAmount != 0){
							...
							$("#tdspdfreport").removeClass("hidden");
						}else{
							$("#noListAvailable").html(errors);
							
							$("#tdspdfreport").addClass("hidden");
							$("#norecords").removeClass("hidden");
							$("#fy_details").addClass("hidden");
							$("#totaldiv1").addClass("hidden");
						}
