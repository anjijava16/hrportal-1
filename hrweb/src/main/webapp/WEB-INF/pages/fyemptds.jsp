...
<html>
	...
	<script>
		$(document).ready(function(){
			$("#pageTitle").html("TDS <b class='saptaColor'>Report</b>");
			...
			$(document).on('click', '.fymonthlist' ,function(){
				...
				var resourceURL = $("#contextpath").val()+"/emptds/viewfytds/"+monthYearVal;
			...
			$("#tdspdfreport").click(function() {
				...
				document.location.href =  $("#contextpath").val()+"/emptdspdfreport?startDate="+startDate+"&endDate="+endDate;
			});
			
			function finalYearReportProcess(currentdate, nextYear){
				$("#block_overlay").removeClass("hidden");
				var completeTotalAmount = 0;
				var resourceURL = $("#contextpath").val()+"/fyreports/getemptdsreport/"+currentdate+"/"+nextYear;
				$.ajax({
					url : resourceURL,
					type : 'GET',
					dataType : 'json',
					success: function(data) {
						...
						if(successflag == "true"){
							...
							$.each(results, function (i, result){
								dueDate.push(result.tdsdatemonth);
							});
							var dueAmount = 0;
							var count = 1;
							var flag = 1;
							...
							for(var i=0; i<newDueDate.length; i++){
								...
								var resultCount = 0;
								$.each(results, function (i, result){
										var fullDueDateFormated = stringToDateConversion(result.tdsdatemonth);
