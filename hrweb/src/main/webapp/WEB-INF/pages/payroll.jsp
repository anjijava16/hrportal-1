	...
	<body>
		...
		<div id="innerContainer">
			<div id="contentArea" style="overflow: hidden;">
				...
				<!-- <input type="button" value="Export" class="leftElement MRGL10PX" id="exportData" style="margin:4px 0px 6px 0px; float:right;"/> -->
	...
	<script>
		...
		$(document).ready(function(){
			...
			$("#findByMonth").click(function() {
				...
				$.ajax({
			        ...
			        dataType : 'json',
			        success: function(data) {
			        	$("#currenttotalamttd").html(numberWithCommas((data).toFixed(2)));
			        	if(data == 0){
			        		$("#exportData").addClass("hidden");
			        	}else{
			        		$("#exportData").removeClass("hidden");
			...
			$("#sendPaysliptab").click(function() {
				...
				var currentdate = monthConversion(currentmonth);
				currentdate = currentdate.split("/").join("-");
				var base =  $("#contextpath").val()+"/sendpayslip?employee="+employee+"&currentmonth="+currentdate+"&strcurrentmonth="+currentmonth+"&sendpayslip=sendpayslip";				
				$.ajax({
					url : base,
					type : 'GET',
					dataType : 'json',
					...
					success: function(data){
			        	...
			        	if(errors == "true" || errors != "" ){
			        		$("#userMsgContent").html("Sending payslip failed...");
			        		$.fancybox.open('#userMsg');
			        		$("#datedisable").addClass("hidden");
				    		$("#optiondiv").removeClass("hidden");
			        	}
		...
		$("#dayspayable").bind('keyup mouseup', function () {
			...
			if($("#dayspayable").html() != ""){
				...
				/* var dynamiclop = daysinMonth- numberofdayspayable;
			
				if( dynamiclop > 0) {
					$("#lop_table").show();
					$("#lossofpay").html(dynamiclop);
				}else{
					$("#lop_table").hide();
				} */
		...
		$("#tds").bind('keyup mouseup', function () {
		        	...
		        	if(successflag == "true"){
						...
						$.ajax({
							...
							success: function(data){
					        	...
					        	if(errors == "true" || errors != "" ){
					        		$("#userMsgContent").html("Salary has been paid and <b style='color: #B81E25'>Sending payslip failed...</b>");
					        		$.fancybox.open('#userMsg');
					        		$("#datedisable").addClass("hidden");
						    		$("#optiondiv").removeClass("hidden");
					        	}
						...
						if(successflag == "true"){
							...
							$.each(results, function (i, result) {
								...
								var pf = 0.00;
								 ...
								 // upto 2,50,000 - No Tax :: 2,50,001 to 5,00,000 - 10% :: 5,00,00 - 10,00,000 - 20%, Above 10,00,001 - 30%
								
								/* if(ctc >= 1000000){
									tds = ((mctc * 30) / 100).toFixed(2);
									$("#tds").html(parseFloat(tds).toFixed(2));
								}else if(ctc >= 500000){
									tds = ((mctc * 20) / 100).toFixed(2);
									$("#tds").html(parseFloat(tds).toFixed(2));
								}else if(ctc >= 250000){
									tds = ((mctc * 10) / 100).toFixed(2);
									$("#tds").html(parseFloat(tds).toFixed(2));
								}else{
									$("#tds").html("");
								} */
								...
								if(joiningdate.substr(3) == selecteddatevalue.substr(3)){
									var dayspay= daysinMonth - (loseofpay + parseInt(joiningdate.substr(0,2))) + 1;
									$("#dayspayable").html(dayspay);
									var finallamount = perdaysalary * dayspay;
								}else{
		...
		function montlySalCalculator(monltlyamount){
			...
			var totaldeduction =  parseFloat(mpf) + parseFloat(mpt) + parseFloat(tds);
			...
			$("#decline").click(function(){
			 ...
			 $("#exportData").click(function() {
				var selectedMnth = $("#payrollmonth").val();
				var selectedMnth = monthConversion(selectedMnth);
				selectedMnth = selectedMnth.split("/").join("-");
				document.location.href = $("#contextpath").val()+"/payrollexport?month="+selectedMnth;
			});
	</script>
</html>
